...
if(!nav.some(x=>x[0]==='path'))nav.splice(5,0,['path','Образовательный маршрут','↗']);
function learningPath(){return `<div class="page">${title('Образовательный маршрут','Ваш план развития: предметы, кружки и портфолио')}<div class="grid two-col"><div class="card"><h2>Мой маршрут · 8 «А»</h2><div class="assessment"><span class="circle">1</span><div class="lesson-main"><b>Академическая база</b><small>Алгебра, биология и английский · 3 из 5 тем завершены</small></div></div><div class="assessment"><span class="circle">2</span><div class="lesson-main"><b>Дополнительное направление</b><small>Робототехника, дебаты, Model UN или олимпиады</small></div></div><div class="assessment"><span class="circle">3</span><div class="lesson-main"><b>Портфолио достижений</b><small>Проекты, мероприятия и волонтёрство</small></div></div></div><div class="card accent-card"><div class="eyebrow">СЛЕДУЮЩИЙ ШАГ</div><h3>Выберите одно дополнительное занятие</h3><p class="sub">Рекомендуем робототехнику: она сочетается с математикой и проектной работой.</p><button class="btn small" data-page="community">Посмотреть кружки</button></div></div></div>`}
const baseRender=render;render=function(){if(state.page==='path'){document.body.classList.toggle('dark',state.theme==='dark');$('#role').value=state.role;$('#avatar').textContent=state.role==='teacher'?'Г':'А';$('#points').textContent=state.points.toLocaleString('ru-RU');$('#nav').innerHTML=nav.map(x=>`<button class="nav-btn ${state.page===x[0]?'active':''}" data-page="${x[0]}"><span class="ico">${x[2]}</span>${x[1]}</button>`).join('');$('#crumb').textContent='Образовательный маршрут';$('#content').innerHTML=learningPath();return}baseRender()};
document.addEventListener('click',e=>{if(e.target.closest('.points'))modal(`<h2>NIS Points · ${state.points.toLocaleString('ru-RU')}</h2><p>NIS Points — баллы активности. Их получают за домашние задания, мероприятия, проекты и волонтёрство.</p><div class="card accent-card"><b>Как получить баллы</b><p class="sub">+30 за выполненное ДЗ · +20 за мероприятие.</p></div><div class="modal-actions"><button class="btn" data-close>Понятно</button></div>`);},true);
async function syncBookings(){try{const r=await fetch('/api/bookings');state.bookings=(await r.json()).bookings||[];save();if(state.page==='rooms')roomCards()}catch{}}
document.addEventListener('submit',async e=>{if(e.target.id!=='booking-form'||state.role!=='teacher')return;e.preventDefault();e.stopImmediatePropagation();const d=new FormData(e.target);try{const r=await fetch('/api/bookings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({room:d.get('room'),time:d.get('time'),purpose:d.get('purpose')})});const x=await r.json();if(!r.ok)throw Error(x.error);state.bookings.push(x.booking);save();closeModal();roomCards();toast('Кабинет забронирован для всех пользователей')}catch(x){toast(x.message)}},true);document.addEventListener('click',e=>{if(e.target.closest('[data-page="rooms"]'))setTimeout(syncBookings,0)});syncBookings();render();
